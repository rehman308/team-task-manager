...
export default function AdminUsers() {
  ...
  const isFormValid = ({ name, email, password, role }) => {
    ...
    if (errors.name || errors.email || errors.password) {
      setFormError({ name: errors.name, email: errors.email, password: errors.password });
    } else {
  ...
  return (
    <>
      ...
      <div className='min-h-screen max-w-6xl mx-auto flex flex-col'>
        ...
        {/*Form to edit username email and password */}
        ...
        {/* Table showing all users for admin to manage */}
          ...
          <tbody>
            {users.map((u) => (
              <tr
                ...
                onClick={user.id !== u.id ? () => navigate(`/admin/users/${u.id}/projects`) : null}>
                ...
                <td className='p-2 border-b-1 flex justify-evenly'>
                  ...
                  {/*So the current user cannot delete himself, hides the delete button*/}
                  {user.id !== u.id && (
                    <Button
                      label='Delete'
                      color='bg-red-500'
